...
@test "capacity: probes both g6e types in the default region (us-east-1) with profile cohack" {
  ...
  [ "$status" -eq 0 ]
  [[ "$output" == *"g6e.xlarge us-east-1a: AVAILABLE"* ]] || return 1
  [[ "$output" == *"g6e.2xlarge us-east-1a: AVAILABLE"* ]] || return 1
...
@test "capacity-log: prints the last n lines and a per-type/zone availability summary" {
  ...
  [[ "$output" == *"g6e.xlarge us-east-1a"*"1/2"* ]] || return 1
  [[ "$output" == *"g6e.2xlarge us-east-1b"*"0/1"* ]] || return 1
