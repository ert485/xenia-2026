...
- Status: **v2.5, approved by Erik on 2026-09-24** after his review, one fresh adversarial review of the whole spec, two focused fresh reviews of the principles (a teammate persona and a mechanism check), and a public-exposure review. Next step: `superpowers:writing-plans`.
...
| `06-onboarding.md` | Cloud kit for teammates | Model disclosure first, then the core principles with the line that they're everyone's to change and Erik only filled in the initial state, then one plain sentence that the repo, issues, PRs, and the kit site are public and carry your GitHub name, then the cloud kit. Accept invite, open dev container, paste your key, run `claude`, push to a branch, get a preview URL. The skills (`/pain`, `/rule-feedback`, `/notify`, `/preview`, `/shutdown-entry`, `/demo-checklist`). Cheap mode, OpenCode fallback, Kiro note (students get 1,000 credits/month), bring-your-own-host path, Python path, reading box logs with `scripts/logs.sh`, where product API keys live. |
...
xenia-2026/                     # public, MIT
  ...
  plugin/           # kit plugin, vendored into the team repo by onboard-repo.sh: skills (/pain, /rule-feedback, /notify, /preview, /shutdown-entry, /demo-checklist) and the SessionStart hook
