...
| Thu | **00b**: the management-account root has MFA but it's an authenticator app (checked 2026-09-24); add a passkey or hardware key as a second root device, and register one on your Identity Center user too (Identity Center, Users, your user, MFA devices), then set Identity Center's MFA mode to every sign-in with security keys allowed; in the kit repo's security settings turn on the non-provider secret patterns if the option is shown; optionally set this repo's commit email to your GitHub noreply address | The management account holds the card |
